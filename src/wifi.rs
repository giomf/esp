...
impl<'a> Wifi<'a> {
    pub fn new(event_loop: EspSystemEventLoop, modem: Modem) -> Result<Self> {
        ...
        let driver = WifiDriver::new(modem, event_loop.clone(),Some(EspDefaultNvsPartition::take()?))?;
        ...
        let wifi = EspWifi::wrap_all(driver, network_configuration, EspNetif::new(esp_idf_svc::netif::NetifStack::Ap)?)?;
    ...
    pub fn get_hostname(&self) -> Result<String> {
        Ok(self.wifi.wifi().sta_netif().get_hostname()?.to_string())
    }
}
